...
class SkillRateCardCreate(models.TransientModel):
    ...
    def create_rate_card(self):
        ...
        for lines in rate_card:
            if lines.delivery_site.id == self.delivery_site.id and lines.project_skill.id == self.project_skill.id and lines.years_of_exp_min == self.years_of_exp_min and lines.years_of_exp_max == self.years_of_exp_max:
                # raise ValidationError(_("Rate Card already present with same values") )
                duplicate_value = True
            else:
                duplicate_value =False
            if duplicate_value == True:
                break
