...
class InvBalSumWiz(models.TransientModel):
    ...
    def action_open_report(self):
        ...
        if not len(self.location_ids):
            ...
            + coalesce((SELECT SUM(sml.qty_done) FROM stock_move as sm INNER JOIN stock_move_line as sml on sml.move_id = sm.id WHERE sm.date < '{}' AND sm.product_id = ibs.product_id AND sml.location_dest_id in {} AND sm.picking_type_code = 'internal'),0)
            - coalesce((SELECT SUM(sml.qty_done) FROM stock_move as sm INNER JOIN stock_move_line as sml on sml.move_id = sm.id WHERE sm.date < '{}' AND sm.product_id = ibs.product_id AND sml.location_id in {} AND sm.picking_type_code = 'internal'),0),
