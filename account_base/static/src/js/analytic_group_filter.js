odoo.define('analytic_group_filter.analytic_group_filter', function (require) {
'use strict';
...
var SearchSalesperson = AccountReport.include({
    ...
    custom_events: {
        'value_changed': function(ev) {
             ...
             self.report_options.salespersion_ids = ev.data.salespersion_ids;
    ...
    render_searchview_buttons: function() {
        ...
        if (this.report_options.partner) {
            if (!this.M2MFilters) {
                ...
                if ('salespersion_ids' in this.report_options) {
                    fields['salespersion_ids'] = {
                        label: _t('Sales Persons'),
                        modelName: 'res.users',
                        value: this.report_options.salespersion_ids.map(Number),
                    };
                }
                if (!_.isEmpty(fields)) {
        ...
        if (this.report_options.analytic) {
            if (!this.M2MFilters) {
                ...
                if (this.report_options.analytic_groups) {
...
// core.action_registry.add('account_report', SearchSalesperson);
