...
class BtAsset(models.Model):   
    ...
    def _get_default_location(self):
        ...
        if not obj:
            raise UserError(_("Please create asset location first"))
        loc = obj[0]
        return loc 
...
class BtAssetCategory(models.Model): 
    ...
    asset_category_seq = fields.Many2one('ir.sequence',string="Sequence")
...
class AccountAsset(models.Model):
    ...
    asset_code = fields.Char(string='Asset Code')
    asset_category_id = fields.Many2one('bt.asset.category', string="Asset Category")
    asset_cat = fields.Char(related='asset_category_id.name', store=True, string="Category")
    ...
    def create(self, vals):
        # print("sfjseu****************")
        # pdb.set_trace()
        sequence =  vals.get('asset_category_id')
        sequence_type = self.env['bt.asset.category'].search([('id','=',sequence)])
        if sequence_type:
            vals['asset_code'] = sequence_type.asset_category_seq.next_by_id()
