...
class BtAssetMove(models.Model):
    ...
    from_loc_id = fields.Many2one('bt.asset.location', string='From Location')
    asset_id = fields.Many2one('bt.asset', string='Asset', copy=False)
    to_loc_id = fields.Many2one('bt.asset.location', string='To Location')
    state = fields.Selection([
            ('draft', 'Draft'),
            ('done', 'Done')], string='State', default='draft', copy=False)
