...
class ReportAccountAgedPartnerInherit(models.AbstractModel):
    ...
    def _get_sql(self):
        ...
        query = ("""
            ...
            SELECT
                ...
                move.invoice_date AS reference_date,
            ...
            JOIN account_payment_term payment ON payment.id = move.invoice_payment_term_id
            ...
            GROUP BY account_move_line.id, partner.id, trust_property.id, journal.id, move.id, account.id, payment.id,
    ...
    def _get_column_details(self, options):
        ...
        if options.get('filter_account_type') == 'receivable':
            ...
            self._field_column('reference_date', name="Invoice Date"),
            self._field_column('payment_term'),
            ...
            self._header_column(),
            self._field_column('report_date'),
            self._field_column('reference_date', name="Bill Date"),
            self._field_column('payment_term'),
            self._field_column('journal_code', name="Journal"),
            self._field_column('account_name', name="Account"),
            self._field_column('expected_pay_date'),
