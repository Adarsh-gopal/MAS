...
class AccountMove(models.Model):
    ...
    def action_post(self):
        if self.move_type in ('in_invoice','in_refund'):
            for line in self.line_ids.filtered(lambda line: line.product_id.id != False and not line.tax_line_id.id and line.product_id.categ_id.purchase_account_id.id and line.product_id.categ_id.purchase_offset_account_id.id):
                # print("\n\n{}\n\n".format(line.product_id.id))
                self.line_ids = [(0, 0, {
                    'account_id': line.product_id.categ_id.purchase_account_id.id,
                    'name': line.name,
                    'debit': line.price_subtotal if self.move_type == 'in_invoice' else 0,
                    'credit': line.price_subtotal if self.move_type == 'in_refund' else 0,
                    'exclude_from_invoice_tab':True,
                    'purchase_offset_line':True
                }),(0, 0, {
                    'account_id': line.product_id.categ_id.purchase_offset_account_id.id,
                    'name': line.name,
                    'debit': line.price_subtotal if self.move_type == 'in_refund' else 0,
                    'credit': line.price_subtotal if self.move_type == 'in_invoice' else 0,
                    'exclude_from_invoice_tab':True,
                    'purchase_offset_line':True
                })]
